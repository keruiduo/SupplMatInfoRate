---
title: "The bio-cultural niche of language is optimised for information transmission"
...
knitr::opts_chunk$set(echo = TRUE, fig.align="center", warning=FALSE, message=FALSE,
                      fig.width=9, fig.height=6, fig.align = "center", comment=NA,
...
info.rate.data <- read.table("./InfoRateData(2017).csv", header=TRUE, sep="\t", quote="", stringsAsFactors=FALSE);

# Add language family data:
info.rate.data$LgFam <- Vectorize(function(a) # assign language family to langauge codes
{
...
info.rate.data$Code_Spker <- as.factor(info.rate.data$Code_Spker)
...
# SpeechRate -> SR
# ShannonEntropy -> ShE
# ConditionalEntropy -> ID
# InfoRate_ShannonEntropy -> ShIR
# InfoRate_ConditionalEntropy -> IR
# LgFam -> Family
names(info.rate.data) <- c("Language", "Text", "Spker", "Speaker", "Sex", "Duration", "NS", "SR", "ShE", "ID", "ShIR", "IR", "Family")
```


## Introduction

This `Rmarkdown` script (and corresponding TAB-separated CSV input data file `InfoRateData(2017).csv` and the resulting `HTML` document) contain the full analysis and plotting code accompanying the paper *The bio-cultural niche of language is optimised for information transmission*.
...
info.rate.data.long <- melt(info.rate.data, id.vars=c("Language", "Text", "Spker", "Speaker", "Sex", "Family"));
...
The data is structured as follows:
  ...
  - *Language* is the unique language ID with `r length(unique(info.rate.data$Language))` possible values `r paste0('"',sort(unique(info.rate.data$Language)),'"',collapse=", ")`;
  ...
  - *Spker* and is the unique speaker ID with possible values: `r length(unique(info.rate.data$Spker))` -- a speaker might have produced texts in more than one language or the same code might have been use more than once;
  - *Speaker* is the concatenation of the speaker's language and ID (a speaker may produce more than a single language): `r length(unique(info.rate.data$Speaker))`;
  - *Sex* is the speaker's sex;
  - *Duration* is the texts' duration in seconds as spoken by the given speaker;
  ...
  - *ShE* and *ID* are language-level entropy estimates;
  - *ShIR* and *IR* are text x speaker properties obtained by multiplying *SR* by *ShE* and *ID* respectively.
...
ggplot(info.rate.data, aes(x=Speaker, y=SR, color=Language)) + geom_boxplot() + ylab("SR") + xlab("Speakers") + theme(axis.text.x=element_blank());
```
...
model.NO.no.modelled.sigma.SR <- gamlss(SR ~ 1 + Sex + random(Text) + random(Language) + random(Speaker), 
                                        data=info.rate.data, family=NO(mu.link="identity"),
...
mix.SR.NO.1 <- gamlssMX(SR ~ 1, family=NO, K=1, data=info.rate.data, plot=FALSE)
mix.SR.NO.2 <- gamlssMX(SR ~ 1, family=NO, K=2, data=info.rate.data, plot=FALSE)
mix.SR.NO.3 <- gamlssMX(SR ~ 1, family=NO, K=3, data=info.rate.data, plot=FALSE)
mix.SR.NO.4 <- gamlssMX(SR ~ 1, family=NO, K=4, data=info.rate.data, plot=FALSE)
mix.SR.NO.5 <- gamlssMX(SR ~ 1, family=NO, K=5, data=info.rate.data, plot=FALSE)
```
...
mix.IR.NO.1 <- gamlssMX(IR ~ 1, family=NO, K=1, data=info.rate.data, plot=FALSE)
mix.IR.NO.2 <- gamlssMX(IR ~ 1, family=NO, K=2, data=info.rate.data, plot=FALSE)
mix.IR.NO.3 <- gamlssMX(IR ~ 1, family=NO, K=3, data=info.rate.data, plot=FALSE)
mix.IR.NO.4 <- gamlssMX(IR ~ 1, family=NO, K=4, data=info.rate.data, plot=FALSE)
mix.IR.NO.5 <- gamlssMX(IR ~ 1, family=NO, K=5, data=info.rate.data, plot=FALSE)
```
...
)
{
  ...
  d <- do.call(rbind,lapply(1:nrow(grouping.var.pairs), function(i)
  {
    ...
    c("Kolmogorov–Smirnov"=as.numeric(suppressWarnings(ks.test(measured.var[s1], measured.var[s2])$statistic)),
      "Kullback-Leibler"  =as.numeric(philentropy::distance(h12, method="kullback-leibler")),
      "Jensen-Shannon"    =as.numeric(philentropy::distance(h12, method="jensen-shannon")),
      "Hellinger"         =as.numeric(philentropy::distance(h12, method="hellinger")),
      "Squared-Chi"       =as.numeric(philentropy::distance(h12, method="squared_chi")));
  }));
...
pairwise.dists <- do.call(rbind, lapply(c("NS", "SR", "IR"), function(s)
{
